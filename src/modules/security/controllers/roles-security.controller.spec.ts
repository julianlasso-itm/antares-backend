...
import { RolesSecurityController } from './roles-security.controller';

jest.mock('ulid', () => ({
  ulid: jest.fn(() => 'mock-ulid'),
}));

describe('RolesSecurityController', () => {
  let controller: RolesSecurityController;
  let service: jest.Mocked<RolesSecurityService>;

  const mockRole = new RolesSecurity();
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      controllers: [RolesSecurityController],
      providers: [
        {
          provide: RolesSecurityService,
          useValue: mockService,
        },
      ],
    }).compile();

    controller = module.get<RolesSecurityController>(RolesSecurityController);
    service = module.get(RolesSecurityService);
  });
